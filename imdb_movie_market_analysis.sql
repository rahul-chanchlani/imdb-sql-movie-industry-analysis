...
-- Number of Movies released based on the months
SELECT 
    MONTH(date_published) Month, COUNT(id) number_of_movies
FROM
    movie
...
SELECT 
    genre, COUNT(movie_id) count_of_movie
FROM
    genre
GROUP BY genre
ORDER BY COUNT(movie_id) DESC
LIMIT 1;
-- Genre Drama has the highest movies produced overall
...
WITH temp_genre AS(
	SELECT movie_id, COUNT(genre) AS count_genre 
	FROM genre
	GROUP BY movie_id
	HAVING count_genre = 1)
SELECT 
    COUNT(movie_id) AS one_genre_movie_count
FROM
    temp_genre;
...
SELECT 
	...
	SUM(total_votes) desc) actor_rank
FROM
    names n
        INNER JOIN
...
+---------------+-------------------+---------------------+----------------------+-----------------+*/
-- Type your code below:
-- Top 3 genre based on the nnumber of movies
WITH genre_top_3 AS
( 	
...
-- details of movie based on the worlwide_gross_income
grossing_movies AS
(
	SELECT genre,
			year,
...
-- Top 5 grossing movies each year that belong to the top three genres
SELECT *
...
SELECT 
	name actress_name,
	SUM(total_votes) total_votes,
	COUNT(r.movie_id) count_of_movies,
	AVG(avg_rating) actress_avg_rating,
...
INNER JOIN role_mapping m ON r.movie_id=m.movie_id
...
--------------------------------------------------------------------------------------------*/
-- Type you code below:
-- To get the next movie date using the function LEAD
...
date_difference_publish_next_movie AS
(
	 SELECT *, DATEDIFF(next_movie_date, date_published) AS diff
	 FROM movie_date_information
 ),
 -- to calculate average of the date difference based on name_id
 avg_interdifferenct_days AS
 (
	 SELECT name_id, AVG(diff) AS avg_inter_movie_days
	 FROM date_difference_publish_next_movie
	 GROUP BY name_id
 ),
 -- final directors details
 final_result AS
 (
	 ...
	 FROM
		 ...
		 JOIN movie AS m 
         ON m.id=r.movie_id
		 JOIN avg_interdifferenct_days AS a 
